...
package de.egladil.web.fileshare.infrastructure.filters;
...
public class SPARouteFilter {
  ...
  private static final String APP_PLUS_API_PREFIX = "/fileshare" + API_PREFIX;

  private static final String DEFAULT_APP = "/fileshare/";
    ...
    if (path.startsWith(APP_PLUS_API_PREFIX)) {
      ...
      if (this.doesNotNeedRedirect(path)) {
        ...
        if (path.startsWith(DEFAULT_APP)) {
          ...
          // I0094: deep-Angular-Router-Links (z.B. /fileshare/aufgabensammlungen/)
          // müssen zur SPA Grund-URL
          // (/fileshare/) umgeleitet werden. Danach übernimmt wieder das
          // Angular-Routing
  ...
  boolean doesNotNeedRedirect(final String path) {
    ...
    if (FILE_NAME_PREDICATE.test(path)) {
      ...
      LOGGER
          .debug("(3-2) kein Umleiten von statischen files aus src/main/resources/META-INF/resources/fileshare/");
      return true;
    }
