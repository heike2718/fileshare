    ...
    static RuntimeException map(Response response) {
        ...
        if (status == 401) {
            throw new DownstreamServiceException(
                    "client-Authentifizierung fehlgeschlagen. Mal clientId hier und in der auth-database prüfen",
                    RestClientType.filescanner);
        }

        return new DownstreamServiceException("unerwarteter status " + status + ". Haben wir nicht mit gerechnet",
                RestClientType.filescanner);
    }
}
